...
let ar8 = [];
let arr81 = Array.from(s8);

const f8 = () => {
    for (i = 0; i < arr81.length; i++) {
        if (arr81[i] > 5) {
            ar8.push(arr81[i])
        }
    }
...
const f10 = (out_set, elem) => {
    return Array.from(out_set, elem).join(' ')
}

document.querySelector('.b-10').onclick = () => {
    let a10 = new Set(['4', '5', '6']);
    document.querySelector('.out-10').innerHTML = f10(a10);
    // f10(a10, '.out-10');
};
...
const f13 = () => {
    ...
    let result = [];
    for (let i of set13) {
        result[i] = arr13.filter(s => s === i).length;
    }
    return result;
}
